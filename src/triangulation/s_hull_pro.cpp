...
void circle_cent2(float r1, float c1, float r2, float c2, float r3, float c3, float &r, float &c,
		float &ro2) {
	/*
...
void write_Triads(std::vector<Triad> &ts, char * fname) {
	...
	for (int r = 0; r < nr; r++) {
		out << ts[r].a + 1 << ' ' << ts[r].b + 1 << ' ' << ts[r].c + 1 << ' ' << ts[r].ab + 1 << ' '
				<< ts[r].ac + 1 << ' ' << ts[r].bc + 1 << endl; //" " << ts[r].ro <<  endl;
	}
	out.close();
...
void circle_cent4(float r1, float c1, float r2, float c2, float r3, float c3, float &r, float &c,
		float &ro2) {
	/*
	 *  function to return the center of a circle and its radius
	 * degenerate case should never be passed to this routine!!!!!!!!!!!!!
...
int de_duplicate(std::vector<Shx> &pts, std::vector<int> &outx) {
	...
	for (int k = 0; k < nump - 1; k++) {
		if (dpx[k].r == dpx[k + 1].r && dpx[k].c == dpx[k + 1].c) {
			cerr << "duplicate-point ids " << dpx[k].id << "  " << dpx[k + 1].id << "   at  ("
					<< pts[dpx[k + 1].id].r << "," << pts[dpx[k + 1].id].c << ")" << endl;
...
int T_flip_pro(std::vector<Shx> &pts, std::vector<Triad> &triads, std::vector<int> &slump, int numt,
	...
	for (int t = start; t < numt; t++) {
		...
		if (tri.bc >= 0) {
			...
			int XX = Cline_Renka_test(pts[pa].r, pts[pa].c, pts[pb].r, pts[pb].c, pts[pc].r,
					pts[pc].c, r3, c3);
		...
		if (flipped == 0 && tri.ab >= 0) {
			...
			int XX = Cline_Renka_test(pts[pc].r, pts[pc].c, pts[pb].r, pts[pb].c, pts[pa].r,
					pts[pa].c, r3, c3);
		...
		if (flipped == 0 && tri.ac >= 0) {
			...
			int XX = Cline_Renka_test(pts[pb].r, pts[pb].c, pts[pa].r, pts[pa].c, pts[pc].r,
					pts[pc].c, r3, c3);
...
int Cline_Renka_test(float &Ax, float &Ay, float &Bx, float &By, float &Cx, float &Cy, float &Dx,
		float &Dy) {

	float v1x = Bx - Ax, v1y = By - Ay, v2x = Cx - Ax, v2y = Cy - Ay, v3x = Bx - Dx, v3y = By - Dy,
			v4x = Cx - Dx, v4y = Cy - Dy;
...
int T_flip_pro_idx(std::vector<Shx> &pts, std::vector<Triad> &triads, std::vector<int> &slump,
	...
	for (int x = 0; x < numi; x++) {
		...
		if (flipped == 0 && tri.ac >= 0) {
			...
			int XX = Cline_Renka_test(pts[pb].r, pts[pb].c, pts[pc].r, pts[pc].c, pts[pa].r,
					pts[pa].c, r3, c3);
